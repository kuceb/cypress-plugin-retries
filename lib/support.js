...
Cypress.mocha._mocha.Mocha.Test.prototype.clone = function () {
  ...
  if (Cypress.config('isTextTerminal')) {
    const nestedDepth = getTestNestedDepth(Cypress.currentTest)

    cy.now('task', 'logRetry', [Cypress.currentTest.title, `${ret._currentRetry + 1}/${ret._retries}`, nestedDepth], { log: false })
    .catch((e) => {})
  }
